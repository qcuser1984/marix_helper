...
def recovered_sps_to_df(path_to_sps):
    ...
    if os.path.exists(path_to_sps) and os.stat(path_to_sps).st_size>0: #check if file exists and not empty
        ...
        try:
            df_out = pd.read_fwf(path_to_sps,
            skiprows = rows_to_skip,
            colspecs = [(0,1),(2,8),(12,18),(23,24),
                        (40,46),(47,55),(57,65),
                        (71,74),(74,80),(80,86),(86,89),
                        (89,95),(95,98),(98,101),(101,106)],
            names = ['code','line','point','index',
                     'w_depth','easting','northing',
                     'deploy_jd','deploy_time','node_heading','recov_jd',
                     'recov_time','deploy_year','recov_year','bumper'])
            df_out['deploy_time_str'] = df_out['deploy_time'].astype(str)
            df_out['deploy_time_str'] = df_out['deploy_time_str'].apply(lambda x: x.zfill(6))
            df_out['deploy_jd_str'] = df_out['deploy_jd'].astype(str)
            df_out['deploy_jd_str'] = df_out['deploy_jd_str'].apply( lambda x : x.zfill(3))
            ...
            df_out.drop(columns = ['recov_time_str','deploy_time_str','recovery_datetime_str',
                                  'deploy_datetime_str','deploy_jd_str','recov_jd_str'], inplace =True)
...
def source_sps_to_df(path_to_sps):
    ...
    if os.path.exists(path_to_sps) and os.stat(path_to_sps).st_size > 0:
        ...
        try:
            df_out=pd.read_fwf(path_to_sps,
            skiprows = rows_to_skip,
            colspecs = [(0,1),(2,8),(12,18),(23,24),(30,34),
                        (40,46),(47,55),(56,65),
                        (65,71),(71,74),(74,80),(74,87),
                        (88,92),(92,95),(95,97),
                        (97,98),(98,99),(99,100),
                        (100,101),(101,102),(103,108)],
            names = ['code','line','point','index','gun_depth',
                    'w_depth','easting','northing',
                    'tide','sp_jd','sp_time','sp_time_ms',
                    'sequence','azimuth','sp_year',
                    'depth_edit','timing_edit','pressure_edit',
                    'repeatability','positioning','dither'])
...
if __name__ == "__main__":
    my_sps=r'/qc/06-ARAM/nav/preplot.s01'
    my_rps=r'/qc/06-ARAM/nav/preplot.r01'
    my_dep_rps = r'/qc/06-ARAM/nav/Postplot_R/R_Deployed/BR001522_Deploy_progress.rps'
    my_recov_rps = r'/qc/06-ARAM/nav/Postplot_R/R_Recovered/MT1001021_Recovery_progress.rps'
    my_sps_clean = r'/qc/06-ARAM/nav/Postplot_S/All_Seq_Clean.s01'
    my_sps_raw = r'/qc/06-ARAM/nav/Postplot_S/All_Seq_Raw.s01'
    main()
